          ...
          <fieldset className="mt-2 text-black dark:text-white ">
            <div className="xs:grid xs:grid-cols-2 sm:grid-cols-4 sm:items-center sm:pl-24 sm:text-base sm:justify-center sm:mb-2">
              {categories.map(category => (
                <div key={category.id} className="flex items-center xs:mb-4 sm:mb-0">
                  <input
                    onClick={handleClick}
                    id={category.id}
                    type="radio"
                    name="category"
                    defaultChecked={
                      category.id === 'all' ? category.id === 'all' : category.id === query.category
                    }
          ...
          <>
            {isLoading ? (
              ...
              <div className="xs:hidden sm:grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
                {filteredProjects &&
                  filteredProjects.map(project => <XSmall key={project.title} project={project} />)}
              </div>
            )}
          </>
          <>
